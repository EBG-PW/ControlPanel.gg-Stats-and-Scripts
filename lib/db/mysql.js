...
let GetFailedJobsSinceLastCheck = function (TimeCheckInterval) {
	return new Promise(function (resolve, reject) {
		db.getConnection(function (err, connection) {
			...
			connection.query(`SELECT COUNT(*) FROM ${process.env.DB_DATABASE}.failed_jobs WHERE failed_at >= NOW() - INTERVAL ${TimeCheckInterval} Second;`, function (err, rows, fields) {
				connection.release();
				if (Object.entries(rows).length === 0) {
					resolve("None");
				} else {
	...
	return new Promise(function (resolve, reject) {
		db.getConnection(function (err, connection) {
			...
			connection.query('SELECT credits, server_limit, email_verified_at, discord_verified_at, ip, suspended, role FROM users;', function (err, rows, fields) {
				connection.release();
				if (Object.entries(rows).length === 0) {
					reject("You need at least one user in the database");
				} else {
					resolve(rows);
				}
			});
